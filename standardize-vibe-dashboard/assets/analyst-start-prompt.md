...
- Put only dashboard SELECT queries in `sql/`; one query per `.sql` file; name files as `<schema>.<table>.<query_name>.sql`; no DDL, grants, dbt, contracts, Helm, or docs folders for the first version.
- Build the service as React + FastAPI + ClickHouse.
